...
**背景**：主界面栏目开关原先叫「首页栏目」，藏在「外观 → 界面布局」里，不够直观；且只有 6 个推荐栏目的开关，**顶部卡片（每日推荐 / 我喜欢的音乐 / 私人 FM）无法单独关闭**，栏目名也不随音乐源变化。

**改动**

| 范围     | 内容                                                                                                                                                                               |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 设置入口 | 「外观 → 界面布局」中的条目更名为 **主界面设置**，描述与搜索关键词补齐（主界面 / 首页 / 栏目 / 显示 / 隐藏 / 推荐 / 歌手 / MV / 播客）                                             |
| 配置项   | `homePageSections` 扩展为 9 项：**每日推荐 / 我喜欢的音乐 / 私人 FM**（顶部卡片，仅开关）+ **推荐歌单 / 雷达歌单 / 歌手推荐 / 推荐 MV / 推荐播客 / 新碟上架**（可拖动排序 + 开关） |
| 弹窗     | `HomePageSectionManager.vue` 改为**分组展示**（顶部卡片 / 推荐栏目），带说明文案；栏目名经 `sectionTitle()` 随当前音乐源显示（如「酷狗音乐专属歌单」「QQ 音乐新碟上架」）          |
| 首页     | `HomeOnline.vue` 顶部的每日推荐 / 我喜欢的音乐 / 私人 FM 均受开关控制；全关时顶部区域整体隐藏；缺失栏目的旧持久化数据由 `ensureHomePageSections()` 自动补全                        |
| 复用     | `sectionTitle()` / `isHomeSectionVisible()` / `ensureHomePageSections()` 统一放在 `src/api/recommend.ts`，首页与设置弹窗共用同一套命名与判定                                       |

**验证**：`vue-tsc` EXIT=0、Prettier 通过。
...
- ✅ **`*.vercel.app` 预览/部署链接已关闭**：Vercel 的 Standard Protection 只豁免 **production 域名**，而本项目两个自定义域都是**分支域**（productionBranch=`dev`，无 production 部署）→ 实测开启保护会连自定义域一起 302，不满足「只留自定义域」。故改为在 `vercel.json` 加 **Host 级重定向**：命中 `.*\.vercel\.app` 的请求 307 跳转到 `https://music.ciallo.sale/$1`，自定义域访问不受影响。
  ...
  > **实测**：新部署 URL、分支别名、以及项目主域名 `s-player12-delicateducks-projects.vercel.app`（已用一次生产部署覆盖为带规则的新构建）均返回 **307 → `https://music.ciallo.sale/`**；三个自定义域均为 **200**。**历史已构建的旧部署 URL 仍可直连**（Vercel 部署不可变、规则随构建生效），如需连带关闭见 AUDIT「复审建议」。
