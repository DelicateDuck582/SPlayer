...
客户端的**默认（主）地址**已迁移到 `delicateduck.xyz` 自有域（旧 `duckgame-play.top` 域名保留在历史记录中，仍可用但不再是默认）：
...
| 网易云 API  | `https://music-api2.delicateduck.xyz`（api-enhanced） | 关联 `s-player12` | 可在「音乐源 → API 源」切换              |

实测（2026-09-25）：`music-api-kugo.delicateduck.xyz/search/hot` → `{"status":1,...}` ✓。
...
6. 酷狗「榜单 / 新歌速递 / 歌单广场」已封装但暂无独立页面入口，可作为下一步（接入现有 Discover 或新增「酷狗」浏览页）。
7. 历史遗留：本项目曾用 `feat/api-enhanced` 分支部署自定义域；切到 `NEWAPI` 后自定义域才是新版本。
