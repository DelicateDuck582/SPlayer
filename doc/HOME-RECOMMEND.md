...
2. **主界面设置**（设置 → 外观 → 界面布局 → **主界面设置**）：可显示/隐藏并排序以下栏目
   - 顶部卡片：每日推荐 / 我喜欢的音乐（仅网易云）/ 私人 FM（仅网易云）——仅开关，全关后顶部区域整体隐藏；
   - 推荐栏目：推荐歌单、雷达歌单（仅网易云）、歌手推荐、推荐 MV、推荐播客（仅网易云）、新碟上架——可拖动排序 + 开关。
     弹窗内栏目名会随当前音乐源显示（如「酷狗音乐专属歌单」「QQ 音乐新碟上架」）；旧版本持久化数据里缺失的栏目会在首页加载时自动补全。
3. **切换源后内容没变**：现在不会了（缓存按源作用域）；若仍是旧内容，`Ctrl+F5` 清掉 sessionStorage 即可。
4. **酷狗 / QQ 的每日推荐为空**：该接口要求登录态（酷狗 `/everyday/recommend`、QQ `/getDailyRecommend`），登录后即可；
   若播放直链报「需要登录」，见 [KUGOU-API.md](./KUGOU-API.md) / [QQ-API.md](./QQ-API.md) 的登录说明。
5. **仍未改造的部分**：「每日推荐」独立页面（`DailySongs.vue`）目前仍走网易云 store；
