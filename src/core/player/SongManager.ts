...
class SongManager {
  ...
  public getOnlineUrl = async (id: number, isPc: boolean = false): Promise<AudioSource> => {
    ...
    if (!songData?.url) {
      debugLog(`🔁 [${id}] 首次取链未返回地址，携带 randomCNIP 重试`);
  ...
  public getUnlockSongUrl = async (
    ...
    for (const r of results) {
      if (r.status === "fulfilled" && r.value.success) {
        ...
        debugLog(`最终音质判断：详细输出：`, { unlockUrl, quality });
        return {
          id: songId,
          url: unlockUrl,
          isUnlocked: true,
  ...
  public getAudioSource = async (song: SongType, forceSource?: string): Promise<AudioSource> => {
    ...
    if (song.type === "streaming" && song.streamUrl) {
      ...
      debugLog(`🔄 [${song.id}] Stream URL:`, finalUrl);
      return {
        id: song.id,
        url: finalUrl,
        isUnlocked: false,
    ...
    try {
      ...
      if (forceSource && forceSource !== "auto") {
        ...
        if (unlockUrl.url) {
          if (import.meta.env.DEV)
            debugLog(`🔓 [${songId}] 指定源解锁成功: ${forceSource}`, unlockUrl);
          return unlockUrl;
        } else {
          // 指定源失败，不回退
          return { id: songId, url: undefined };
      ...
      if ((!forceSource || forceSource === "auto") && canUnlock) {
        ...
        if (unlockUrl.url) {
          debugLog(`🔓 [${songId}] 解锁成功`, unlockUrl);
          return unlockUrl;
        }
      }
      ...
      if (!forceSource || forceSource === "auto") {
        ...
        if (fallbackUrl) {
          if (import.meta.env.DEV)
            debugLog(`🚀 [${songId}] 网络请求失败，使用本地缓存兜底`, fallbackUrl);
          return {
            id: songId,
            url: fallbackUrl,
            isUnlocked: true,
