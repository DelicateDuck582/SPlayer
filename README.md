...
| 日期 | 变更摘要 | 详情 |
| --- | --- | --- |
| 2026-09-19 | **Web 端（Vercel）安全 / 密钥 / 性能审计**：修复登出后凭据残留（其余 Cookie 的 `localStorage` 副本 + 云盘上传令牌未清）；CSP 加固（补 `object-src` / `base-uri` / `form-action`）；消息中心首屏请求 4→1（通知类接口改懒加载）。核查：密钥扫描 0 命中、`v-html` 无可用注入点、定时器均有清理。另发现主站域名仍绑定旧分支（`feat/api-enhanced`）且 Vercel 部署保护开启 | [CHANGELOG § Web 端审计](./doc/CHANGELOG.md#v2026-09-19-web-audit) · [AUDIT § Web 端审计](./doc/AUDIT.md) |
