...
- **风控规避**：Electron 端上传的元数据请求优先走**本机内置 API**（`127.0.0.1:25884/api/netease`，即用户本机网络出口），避免在线 API 的数据中心 IP 被网易云风控（`-460`）；本机 API 不可用时自动回退在线 API，并对 `-460` 追加 `checkToken=v2` 重试
